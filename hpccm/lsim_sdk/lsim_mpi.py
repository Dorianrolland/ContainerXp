...
def mpi(tc):
  ...
  Stage0 = hpccm.Stage()
  Stage0.name = 'mpi'
  # Utiliser une image de base compatible avec ppc64le
  Stage0.baseimage(image='nvidia/cuda-ppc64le:12.4.0-devel-centos7',_distro=distro)
...
if __name__ == '__main__':
  ...
  stage, tc = toolchain()
  print(stage)
  print(footer(mPour résoudre les problèmes d'absence de `gcc` et `gcc-c++`, ainsi que pour corriger les avertissements liés à la plateforme (`linux/amd64` au lieu de `linux/ppc64le`), vous pouvez adapter votre script `hpccm` de manière à inclure ces ajustements.

### Script `hpccm` Modifié
...
def mpi(tc):
  ...
  # Installation de gcc et gcc-c++
  Stage0 += packages(apt=['gcc', 'g++'], yum=['gcc', 'gcc-c++'], powertools=True, epel=True)
