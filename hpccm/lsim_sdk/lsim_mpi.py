...
def mpi(tc):
    ...
    if args.mpi == "ompi":
        ...
        if args.target_arch == "x86_64" or args.binary == "no":
            mpi_lib = openmpi(infiniband=True, pmix='internal', version=args.mpi_version, cuda=(args.cuda != 'no'),
                              prefix="/usr/local/mpi", toolchain=tc)
            vars.update({
        ...
        if args.cuda != 'no':
            ...
            if (args.mpi_version == "2.3.6"):
                Stage0 += packages(
                    apt=['cpio', 'libnuma1', 'libpciaccess0', 'openssh-client', 'rpm2cpio', 'libgfortran4'],
    ...
    if args.mpi in ["mvapich2", "mvapich"] and args.cuda != 'no':
        Stage0 += shell(commands=['mkdir /usr/local/mpi/',
                                  'cp -r /opt/mvapich2/gdr/{}/no-mpittool/no-openacc/cuda**/mofed{}/mpirun/gnu{}/* /usr/local/mpi'.format(
                                      args.mpi_version, ofed_version, gnu_version)])
